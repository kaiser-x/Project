...
app.post("/join", (req, res) => {
  ...
  mydata
    ...
    .then(() => {
      res
        .status(200)
        .render(__dirname + "/views/join.html", { message: "success" });
    })
    .catch(() => {
      res
        .status(404)
        .render(__dirname + "/views/join.html", { message: "failed" });
    });
});
...
app.get("/student", (req, res) => {
  fullData.exec((err, data) => {
    if (err) throw err;
    res.render(__dirname + "/views/student.html", {
      record: data,
      message: "",
    });
  });
...
app.listen(port, () => {
  console.log(
    `The application started successfully at : http://${host}:${port}`
  );
});
